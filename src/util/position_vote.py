...
class PositionVote():
    ...
    def generate_random(n_votes, n_cand):
        ...
        log.debug("Computation time to generate random votes: {}".format(time() - start_time))
        if n_votes <= 10:
            log.debug("Generated position votes: " + str(votes))
        return votes

    def __init__(self, position_list):
        """position have to be valid (there has to be a first; if 2 canidates
        are second, noone is third; ...) | [1,3,2] would mean, that cand A is
        first, C second and B third | positions 0 or smaller mean, that the
        candidate is ignored."""
        self.position_list = position_list
        self.n_cand = len(self.position_list)
        self.duel_matrix = None
        self.number_of_ignored = None
        self.first_doubled_position = None
    ...
    def get_duel_matrix(self):
        """generate matrix (numpy array), which indicates who is prefered
        against whom | [[0,0,1],[0,0,1],[0,0,0]] means, that A and B win against
        C but A and B are equal"""
