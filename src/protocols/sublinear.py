...
class VerifyCorrectMulProtocol(Protocol):
    ...
    def run(self, enc_x, enc_y, enc_z, ann, c, res):
        ...
        if not left.val == right.val:
            log.debug('First check of VerifyCorrectMulProtocol fails.')
            return False
        ...
        if not left.val == right.val:
            log.debug('Second check of VerifyCorrectMulProtocol fails.')
            return False
        ...
        try:
            ...
            if not exists(filename):
                ...
                if not str(self.abb.pk.n) in data.keys():
                    generate = True
            if generate:
                log.debug('No data stored for this parameters.')
