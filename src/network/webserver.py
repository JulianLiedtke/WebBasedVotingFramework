...
def check_ssl_certificates_exist():
    if not exists(HTTPSServer.certfile):
        raise ValueError("cert.pem file does not exist. See README instructions how to create one.")
    if not exists(HTTPSServer.keyfile):
        raise ValueError("jey.pem file does not exist. See README instructions how to create one.")
    return True
...
class HTTPSServer:
    ...
    """
    certfile = join('certificate_localhost', 'cert.pem')
    keyfile = join('certificate_localhost', 'key.pem')
