...
class CondorcetEvaluation(EvaluationProtocol):
    ...
    def run(self, vote_aggregation):
        ...
        possible_winner = []
        # https://www.wahlrecht.de/lexikon/borda.html: optimization: condorcet
        # winner should have average borda points
    ...
    def check_candidates(self, possible_winners, search_for_winner):
        new_candidates = [c for c in possible_winners if not c in self.handled_cands]
        ...
        for i, cand_a in enumerate(new_candidates):
            for cand_b in candidates_to_compare[i+1:]:
            ...
            if search_for_winner and self.abb.dec(self.abb.eq(self.strong_gt_sums[cand_a], len(candidates_to_compare) - 1, self.abb.get_bits_for_size(self.n_cand))) == 1:
                log.debug('Needed %s iterations' % (i+1))
                return [cand_a]
