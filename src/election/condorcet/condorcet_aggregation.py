...
class CondorcetVoteAggregation(BaseVoteAggregation):
    ...
    def get_initial_vote_aggregation(abb, n_cand):
        ...
        preference_matrix = [[abb.enc_zero for _ in range(n_cand)] for _ in range(n_cand)]
        doubled_borda_points_sum = [abb.enc_zero for _ in range(n_cand)]

        return [preference_matrix, doubled_borda_points_sum]
