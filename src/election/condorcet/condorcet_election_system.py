...
class Condorcet(ElectionProperties):
    ...
    def generate_valid_vote(self, generic_vote, abb):
        """ generate encrypted gt-matrix and borda points if optimization
        activated out of generic vote """

        gt_matrix_enc = [[abb.enc(e) for e in row] for row in generic_vote.get_duel_matrix()]

        borda_points_enc = []
        if self.leak_better_half:
            borda_winner_points = range(self.n_cand, 0, -1)
    ...
    def deserialize_vote(self, serialized_vote, abb):
        if self.depthCount(serialized_vote["choices"][0]) > self.depthCount(serialized_vote["choices"][1]):
            deserialized_vote = [
                [[abb.deserialize_cipher(vote) for vote in ranking_map] for ranking_map in
                 serialized_vote["choices"][0]],
                [abb.deserialize_cipher(vote) for vote in serialized_vote["choices"][1]]
            ]
        else:
            deserialized_vote = [
                [[abb.deserialize_cipher(entry) for entry in matrix] for matrix in serialized_vote["choices"]],
                []
            ]
        return deserialized_vote

    def depthCount(self, x):
        return 1 + max(map(self.depthCount, x)) if x and isinstance(x, list) else 0
