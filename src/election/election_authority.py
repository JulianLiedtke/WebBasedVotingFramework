...
class ElectionAuthority():
    ...
    def __init__(self, trustee_gen, election_properties: ElectionProperties, bulletin_board_client=None, local_bulletin_board=False, board_id=None):
        ...
        if bulletin_board_client == None:
            bulletin_board_client = BulletinBoardClient(board_id = board_id)

        self.bulletin_board = BulletinBoard() if local_bulletin_board else bulletin_board_client
    ...
    def generate_ballots(self, n_voters, IP):
        """
        Generate all the secrets that allow the users to vote during
        an election, add them to the bulletin board and output them
        into the secrets folder. Only valid for remote bulletin
        boards, as the local versions do not have the address /
        board_id.

        Args:
            n_voters (int): The amount of secrets to generate, i.e.
            how many participants
                will this election have.
            IP (str): The base url of the bulletin board.

        Raises:
            TypeError: If this function is called when the
            bulletin_board is not an 
        ...
        #todo sign hash list and send it than bb saves it 
        secret_list = []
        for i in range(n_voters):
            ...
            self.logger.debug("Adding secret %.8s... with hash %.8s...", secret_hex, hash_hex)
    ...
    def generate_election_configs(self, election_properties):
        ...
        self.logger.debug("Adding election to bulletin board with properties: %s", serialized_properties)
    ...
    def __transform_votes(self, generic_votes):
        ...
        for vote in generic_votes:
            try:
                ...
                self.logger.warning(str(vote) + " couldn't be added, because " + str(e))

        self.vote_transformation_time += time() - start_time
        return valid_votes

    def trigger_evaluation(self):
        self.logger.debug('Computation time to transform votes: {:.3f}s'.format(self.vote_transformation_time))
        ballots = self.bulletin_board.get_votes()
        # TODO move these to where aggregation happens
        # self.logger.debug('Computation time to aggregate votes: {:.3f}s'.format(aggregation_time))
        ...
        self.logger.debug('Computation time to evaluate winner: {:.3f}s, with {} gt-ops, {} eq-ops, {} dec-ops and {} mul-ops'.format(t, gt_ops, eq_ops, dec_ops, mul_ops))
