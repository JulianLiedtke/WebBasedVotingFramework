...
class TrusteeClient(Trustee):
    ...
    def add_on_trustee_server(self):
        url = self.base_url + '/'
        log.debug("Adding trustee on server '%s' with abb '%s'", self.base_url, self.abb.serialize())
...
def init_trustees(bulletin_board, abbs, ids, crashed = False):
    ...
    #if authentication server crashed dont create new trustees load the old ones
    if(crashed):
        with open('data/trustees/trusteeClients_'+str(bulletin_board.board_id)+'.pkl','rb') as f:
                trustees = pickle.load(f)
                for trustee in trustees:
                    #cant save generators so have to restore them after loading file
    ...
    #save the trustee list in case authentication server crashes
    with open('data/trustees/trusteeClients_'+str(bulletin_board.board_id)+'.pkl','w+b') as f:
        #cant save generators so have to temporarily remove them
        for trustee in trustees:
            trustee.abb.rand = None

        pickle.dump(trustees, f, pickle.HIGHEST_PROTOCOL)

        #restore them with same parameters
        for trustee in trustees:
            trustee.abb.rand = gmpy.random_state(randrange(123456789))
