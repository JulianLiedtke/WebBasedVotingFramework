...
class TrusteeWebserver(HTTPSServer):
    ...
    def init_new_trustee(self, board_id, abb, broadcasting_trustee_id, other_trustee_ids):
        ...
        #TODO check if abb is correctly signed or dont save with pickle instead deconstruct trustee and its objects to get values and save with json 
        with open('data/trustees/trustee_'+str(new_id)+'_'+str(self.port)+'.pkl','a+b') as f:
            self.trustees[new_id].abb.rand = None
            pickle.dump(self.trustees[new_id], f, pickle.HIGHEST_PROTOCOL)
            self.trustees[new_id].abb.rand = gmpy.random_state(randrange(123456789))  
        return new_id
...
class _TrusteeHTTPRequestHandler(CustomBaseHTTPRequestHandler):
    ...
    def api_add_trustee(self, board_id, abb, from_id, to_ids):
        ...
        log.debug("Trustee [%s]: Was added for BB '%s'", (self.host_server.port, new_id), board_id)
    ...
    def api_run_protocol(self, trustee_id, protocol, protocol_args):
        self.restore_trustee_if_not_existing(trustee_id)
    ...
    def api_get_status(self, trustee_id):
        ...
        """
        Only a patch for backend not sending correct Data Format in Parliamentary
        """
        if (acting_trustee.result != None):
            if isinstance(acting_trustee.result, type({}.keys())):
                array = []
                for key in acting_trustee.result:
                    array.append(key)
    ...
    def api_trigger_evaluation(self, trustee_id):
        self.restore_trustee_if_not_existing(trustee_id)
        log.debug("Trustee [%s]: %s", (self.host_server.port, trustee_id), "starts evaluation")
        acting_trustee = self.host_server.trustees[trustee_id]
        acting_trustee.trigger_evaluation()
        self.write_headers_and_response(HTTPStatus.OK)

    def restore_trustee_if_not_existing(self, trustee_id):
        if str(trustee_id) in self.host_server.trustees:
            return
        #TODO check if signed correctly
        with open('data/trustees/trustee_'+str(trustee_id)+'_'+str(self.host_server.port)+'.pkl','rb') as f:
                trustee = pickle.load(f)
                trustee.abb.rand = gmpy.random_state(randrange(123456789))
                self.host_server.trustees[trustee_id] = trustee