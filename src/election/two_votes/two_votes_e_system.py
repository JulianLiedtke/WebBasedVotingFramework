...
class FirstVote():
    ...
    def add_random_votes(self, n):
        ...
        for cand in self.cands:
            ...
            log.debug("Kandidat: " + str(cand) + " Stimmen: " + str(rand_vote))
...
class TwoVotesESystem():
    ...
    def run(self):
        ...
        log.debug("Mit Erststimmen gewählte Kandidaten: " + str(winner_first_votes))
        ...
        for id in winner_first_votes:
            ...
            if party not in self.clause_exclusions:
                ...
                log.debug("Partei " + str(party) + " zu Sperrklausel-exclusions hinzugefügt")
        ...
        sum_seats = 0
        for party in range(self.n_parties):
            log.debug("Partei " + str(party))
            if party in seats:
                sum_seats += seats[party]
                log.debug("Sitze " + str(seats[party]))
            else:
                log.debug("Unter Sperrklausel")
            log.debug("--------------------------")
        log.debug("Parteien gewonner Restsitze " + str(residual))
        log.debug("--------------------------")
        ...
        if sum_seats != self.num_seats:
            ...
            log.debug("Sitzzahl aufgrund Gleichstand verändert auf " + str(self.num_seats) + " Sitze")
        ...
        for party, new_seats in balance.items():
            log.debug("Partei " + str(party))
            log.debug("Balancierte Zweitstimmen-Sitzzahl " + str(new_seats))
            log.debug("--------------------------")
    ...
    def add_random_second_votes(self, max, min=0):
        ...
        for party in range(self.n_parties):
            ...
            log.debug("Partei: " + str(party) + " Stimmen: " + str(rand_vote))
...
class CSV_Writer_two_votes:
    ...
    def write(cls, sum_seats, num_parties, secret, failed_parties, residual):
        ...
        log.debug("Succesfully written first time: " + str(first))
        log.debug("Succesfully written second time: " + str(second))
