...
class InstantRunoffEvaluator(Evaluator):
    ...
    def compute_ranking_vector(self):
        ...
        for vote in self.votes:
            ...
            mult_matrix = np.matrix([[self.mult(vote[i,j],i+1) for j in range(self.n_cand)] for i in range(self.n_cand)])

            #sum up the entries of the multiplied matrix in each column
            for col in range(self.n_cand):
                sum = self.encrypt_unified(0)
                for row in range(self.n_cand):
    ...
    def compute_order_vector(self):
        ...
        for vote in self.votes:
            ...
            mult_matrix = np.matrix([[self.mult(vote[i,j], j+1) for j in range(self.n_cand)] for i in range(self.n_cand)])
    ...
    def compute_counter(self, vote):
        ...
        num_rows = self.n_cand - num_act_cand
    ...
    def compute_vote(self, ranking, counter):
        ...
        vote = []
        for rank in ranking:
            vote.append(self.eq(rank, counter))
        return vote
    ...
    def compute_elimination_vector(self, order, loser):
        ...
        elimination_vector = []
        for cand in order:
            elimination_vector.append(self.eq(cand, loser))
    ...
    def decrypt_matrix_for_debug(self, matrix):
        ...
        return [[self.decrypt(matrix[i, j]) for j in range(len(matrix[i]))] for i in range(len(matrix))]