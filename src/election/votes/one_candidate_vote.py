...
class OneCandidateVote(ConstituencyVote):
    ...
    def generate_random_votes(n_votes, n_cand, n_constituencies, type: VoteType = None):
        ...
        for i in range(n_votes):
            ...
            constituency = OneCandidateVote.generate_random_constituency(
                n_constituencies)
            vote = OneCandidateVote(choices, constituency, type)
            votes.append(vote)
        end_time = time()

        log.debug("Computation time to generate random votes: " +
    ...
    def generate_encrypted_votes(abb: ABB, n_votes, n_cand, n_const):
        ...
        """
        start_time = time()
        votes = OneCandidateVote.generate_random_votes(
            n_votes, n_cand, n_const)
        for vote in votes:
    ...
    def generate_random_choice(n_cand):
        """
        Generate random votes voting for one candidate, e.g. [0,0,1,0]
        elects the third candidate.
        """
