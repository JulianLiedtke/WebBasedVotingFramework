...
from src.election.condorcet.condorcet_implementations import MiniMaxMarginsSmith, MiniMaxWinningVotesSmith, Copeland
from src.election.instant_runoff_voting.irv_election_system import Instant_runoff, IRVElectionSystemNormal, IRVElectionSystemAlternative
...
class Authentication:
    def __init__(self):
        ...
        #if no file exist we assume the server is not crashed and this is a new election
        # if the server crashed we need to restore all the election authorities with their auth_tokens and dont generate new ones
        directory = os.fsencode("data/config/")
        for file in os.listdir(directory):
            filename = os.fsdecode(file)
            if filename.endswith(".json") and filename.startswith("as_config"): 
                file_path = "data/config/"+filename
                with open(file_path, 'r') as f:
                    self.isCrashed = True
                    id = filename.split("_")[2][0]
                    raw_config = json.load(f)
                    self.createElection(raw_config, id)
                    f.close()
            else:
                continue
    ...
    def add_ballot(self, vote):
        ...
        log.debug("User with secret %.8s... and hash %.8s... tries to vote", vote["secret"], hash_hex)
        ...
        log.debug("Successful vote, sent ACK '%s' to client.", ack)
    ...
    def createElection(self, raw_config, id=None):
        ...
        def key_generator(bulletin_board): return init_trustees(bulletin_board, abbs, [i for i, _ in enumerate(abbs)], self.isCrashed)
        ...
        # save the raw_config in case authentication server crashes
        if (not self.isCrashed):
            
            self.bulletin_board_client = BulletinBoardClient(base_url=raw_config["communication"]["bulletinBoardServerIP"])
            self.election_id = self.bulletin_board_client.board_id
            self.election_authority = ElectionAuthority(key_generator, election_properties, self.bulletin_board_client)
            self.save_config(raw_config, self.election_id)
        else:
            # if the server crashed we need to restore all the election authorities with their auth_tokens and dont generate new ones
            self.bulletin_board_client = BulletinBoardClient(base_url=raw_config["communication"]["bulletinBoardServerIP"], board_id=id)
            self.election_id = self.bulletin_board_client.board_id
            self.election_authority = ElectionAuthority(key_generator, election_properties, self.bulletin_board_client, board_id=id)
            self.election_authority.bulletin_board.auth_token = self.election_authority.bulletin_board.load_auth_token(self.election_id)

        self.bulletin_board_client.session = self.election_authority.bulletin_board.session

        # if the server crashed we need to restore the auth token and dont generate a new one
        if (self.isCrashed):
            self.bulletin_board_client.session.auth = HTTPBearerAuth(self.election_authority.bulletin_board.load_auth_token(self.election_authority.bulletin_board.board_id))


        # generate Ballots (which already exist if we crashed)
        if (not self.isCrashed):
    ...
    def save_config(self, raw_config, id):
        #save the config with its adress in the name to support multiple server on one computer
        #ip = urllib.parse.quote(raw_config["communication"]["authenticationServerIP"], safe='')
        file_path = "data/config/as_config_"+str(id)+".json"
