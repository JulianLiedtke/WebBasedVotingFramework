...
def simulate_different_irv_votings(key_generator):
    ...
    while(len(sys_ids) > 0):
        ...
        log.debug('start election with the following parameters.')
        log.debug('sys id:    {}'.format(sys_id))
        log.debug('num cand:  {}'.format(num_cand))
        log.debug('num votes: {}'.format(num_vote))
...
if __name__ == '__main__':
    ...
    abbs = PaillierABB.gen_trustee_abbs(64, 3, 2, SubLinearProtocolSuite)
    simulate_different_irv_votings(lambda bulletin_board: init_trustees(bulletin_board, abbs, [i for i, _ in enumerate(abbs)]))
