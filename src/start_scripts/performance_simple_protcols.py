...
if __name__ == '__main__':
    ...
    abbs = PaillierABB.gen_trustee_abbs(bits_key, trustee_count, threshold, SubLinearProtocolSuite)
    key_generator = lambda: init_trustees(BulletinBoard(), abbs, [i for i, _ in enumerate(abbs)])

    if (specific_test > 0):
        test_performance_specific_basic_ops(key_generator)
    else:
        test_performance_basic_ops(key_generator)
