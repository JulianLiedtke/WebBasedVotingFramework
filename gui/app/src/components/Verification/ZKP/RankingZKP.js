 ...
 * @param {JSON} randomNumbers numbers used in the website
 * 
 * @returns {JSON} proof    
 */
function generateProof(cipher, paillierRandomNumber, votes, numberOfCandidates, maxVotes, voterSecret, publicKey, commitment, commitmentPermutationArgument, randomNumbers) {
    ...
    //use the random numbers also used in the website
    var r = bigInt(randomNumbers.r);
    var r_a = randomNumbers.r_a.split(",");
    for(var i = 0; i < r_a.length; i++) {
        r_a[i] = bigInt(r_a[i]);
    }
    var r_r = bigInt(randomNumbers.r_r);
    var R_R = bigInt(randomNumbers.R_R);
    ...
    let {c_d, c_delta, c_a, fBlackbox, zBlackbox, f_deltaBlackbox, z_deltaBlackbox} = permutationArgument(c, r, votes, numberOfCandidates, voterSecret, commitment, commitmentPermutationArgument, randomNumbers);
    ...
    //generate JSON object to return
    var proof = {};
 ...
 * @param {PedersenVectorCommitment} commitmentPermutationArgument vector commitment for vectors of the size numberOfCandidates - 1
 * @param {JSON} randomNumbers random numbers used in the website
 * 
 * @returns {{c_d: bigInt, c_delta: bigInt, c_a: big, fBlackbox: Array.<bigInt>, zBlackbox: bigInt, 
 *              f_deltaBlackbox: Array.<bigInt>, z_deltaBlackbox : bigInt}}
 *          permuation argument that proves that the votes are a permutation of the point distrubution
 */
function permutationArgument(c, r, votes, numberOfCandidates, voterSecret, commitment, commitmentPermutationArgument, randomNumbers) {
    //compute challange
    var aux = CryptoJS.SHA512(CryptoJS.enc.Hex.parse(voterSecret)).toString(CryptoJS.enc.Hex);
    ...
    var r_d = bigInt(randomNumbers.r_d);
    var r_delta = bigInt(randomNumbers.r_delta);
    var r_a = bigInt(randomNumbers.r_aPermutation);
    var d = randomNumbers.d.split(",");
    for(var i = 0; i < d.length; i++) {
        d[i] = bigInt(d[i]);
    }
    var delta = randomNumbers.delta.split(",");
    for(var i = 0; i < delta.length; i++) {
        delta[i] = bigInt(delta[i]);
    }
    ...
    return {c_d, c_delta, c_a, fBlackbox, zBlackbox, f_deltaBlackbox, z_deltaBlackbox};
}
