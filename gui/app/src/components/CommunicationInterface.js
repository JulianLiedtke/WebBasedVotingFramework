...
class CommunicationInterface {
  ...
  /**
   * Delete the key, the message will not be encrypted anymore
   */
  deleteKey() {
    this.key = "";
  }
  ...
  startPeer(
    ...
    //var id = (Math.random() * 10) | 0; // only for dev purpose
    ...
    this.peer.on("connection", conn => {
      ...
      conn.on("close", () => {
        if (this.connection != null) {
          ...
          // So no forbidden connection is calling calback
  ...
  receive(payload) {
    ...
    if (payload.data) {
      ...
      alert(
        "Verbindungsproblem! Bitte Seite neu laden und von vorne beginnen!"
      );
    }
  }
