<template>
  <q-page class="q-ma-md">
    ...
    <div>
      Number of Equal Ranks : {{this.numberOfEqualPositions}},
      Number of Counting Ranks : {{this.numberOfCountingRanks}},
      Point distribution : {{this.pointDistribution}}
...
export default {
  ...
  created() {
    ...
    if(this.numberOfEqualPositions < 0){
      this.equalPositioning = true;
    }
    for (var i = 0; i < this.list.length; i++) {
      this.positions.push({
        points: this.pointDistribution[i]
      });
    }
  },
  computed: {
